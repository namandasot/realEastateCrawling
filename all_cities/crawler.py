...
def getAllCities():
	d = {}
	try:
		response = br.open("https://housing.com/in/buy/")
	except:
		print 'Unable to connect to housing. Check internet connection.'
		sys.exit(1)
	soup = BeautifulSoup(response)
	
	for group in soup.find('div', {'class' : 'all-cities-container'}).findAll('div', {'class' : 'group'}):
		groupCities = group.findAll('li')
		for city in groupCities:
			d[str(city.a.text)] = "https://housing.com" + str(city.a['href'])
	return d	
...
def func(city, link):
	...
	call(["mkdir", city])
	call(["mkdir", city + '/logs'])
	data_file = csv.writer(open(city + '/training_data', 'a'))

	data_file.writerow(header)

	dataIdAccessUrl = 'https://rails.housing.com//api/v2/buy/'
	error_file = open(city + '/error_logs', 'a')
